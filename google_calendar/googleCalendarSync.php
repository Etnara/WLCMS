...
function sync_event_to_google(string $user_id, int $event_id): void {
    ...
    // Load user tokens
    $stmt = $db->prepare("SELECT * FROM dbpersons WHERE id = ?");
    $stmt->bind_param('s', $user_id);
    $stmt->execute();
    $user = $stmt->get_result()->fetch_assoc();

    if (empty($user['google_access_token'])) return; // user hasn't connected Google
    ...
    if ($client->isAccessTokenExpired()) {
        ...
        $stmt->bind_param('sss', $new_token['access_token'], $expires_at, $user_id);
        $stmt->execute();
